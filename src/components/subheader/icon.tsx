...
export default function Icon({ path, children }: Subheader.Icon.Props) {
    ...
    return (
        <div
            ...
            className={`rounded-lg border border-transparent p-2 transition-colors ${shouldHighlight ? 'text-sky-600' : 'text-white cursor-pointer hover:border-slate-700 hover:bg-slate-800/30'}`}>
            {children}
        </div>
    );
}
